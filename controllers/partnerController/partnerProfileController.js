...
exports.getPartnerProfile = async (req, res) => {
  try {
    ...
    const partnerProfile = await PartnerProfile.findOne({ partnerId });
    if (!partnerProfile) {
      return res
        .status(404)
        .json(apiResponse(404, false, "Partner profile not found"));
    ...
    return res
      .status(200)
      .json(apiResponse(200, true, "Partner profile fetched successfully", partnerProfile));
  } catch (error) {
    console.error("Get Partner Profile Error:", error.message);
    return res.status(500).json(apiResponse(500, false, "Server error"));
  }
};
